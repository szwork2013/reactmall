...
W.validate.toggleErrorHint = function(el, isValidate, hint) {
    ...
    var isInputEl = (el.get(0).tagName.toLowerCase() === 'input') || (el.get(0).tagName.toLowerCase() === 'textarea');
    if (isInputEl) {
...
W.validate.reset = function($node) {
    if ($node) {
        if ($node.length === 0) {
            $node = $(document);
        }
