...
W.dialog.Dialog = Backbone.View.extend({
    ...
    initialize: function(options) {
        ...
        //为validate添加星号
        $node.find('[data-validate]').each(function() {
            var $input = $(this);
            $group = $input.parents('.form-group');
            if ($group.length > 0 && !$group.hasClass('xui-nostar')) {
                $group.find('label').eq(0).addClass('xui-validateStar');
            }
        });
...
W.dialog.showDialog = function(dialogName, options) {
    ...
    */
    var dialogDefine = W.dialog.NAME2DEFINE[dialogName];
    if (!dialogDefine.view) {
        xlog('create dialog view for ' + dialogName);
        dialogDefine.view = W.dialog.Dialog.extend(dialogDefine.define);
    } else {
        xlog('use existed dialog view for ' + dialogName);
    }
    var dialogView = dialogDefine.view;
...
W.dialog.define = function(dialogName, dialogDefine) {
    var pos = dialogName.lastIndexOf('.');
    if (pos === -1) {
        xlog("invalid dialog name : " + dialogName);
        return;
    }

    W.dialog.NAME2DEFINE[dialogName] = {
        define: dialogDefine
    }
}